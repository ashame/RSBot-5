...
public class Skills extends Provider {
    ...
    public int getTotalLevel() {
        int level = 0;
        for (int i : ctx.skills.getRealLevels()) {
            level = level + i;
        }
        return level;
    }
    ...
    private int getExperienceRequired(final int lvl) {
        if (lvl > 120) {
            return -1;
        }
        return XP_TABLE[lvl];
    ...
    public int getExperienceToLevel(final int index, final int endLvl) {
        if (endLvl >= XP_TABLE.length) {
            return -1;
        }
        return getExperienceRequired(endLvl) - ctx.skills.getExperience(index);
    }
}
